$(document).ready(function() {
					$('#stats').fadeOut("slow");
					$('#stats').fadeIn("slow");
					$body = $("body");
					$(document).on({
					    ajaxStart: function() { $body.addClass("loading"); },
					    ajaxStop: function() { $body.removeClass("loading"); }
					});
	$("#appendedInputButton").keypress(function(event){ 
    if(event.keyCode == 13){ 
    event.preventDefault();
    $("#singleButton").click(); 
 	}
	});
	$("#singleButton").on("click", function() {
		$("#box").hide();
		var input = $("#appendedInputButton").val();
		//var root = 'http://localhost:3000';
		var root = 'http://sniphunter.sanbi.ac.za';
		$.ajax({
	  	url: root + '/PMCOAI_rs_articles?keywords[]=' + input,
	 	method: 'GET'
		}).then(function(data) {
		var listz = [];
		for(var i=0; i < data.length; i++) {

			var elem = document.createElement("a");
			
			    rsID = data[i].rs_number;
			
				cited = data[i].rs_number_cited_in_article;

				pub = data[i].pubmed_id;
				if (pub ==	"not available") {		
				pub = "N/A";
				};

				year = data[i].publication_date;
				if (year ==	"not available") {		
				year = "N/A";
				};

				doi = data[i].doi;
				if (doi ==	"not available") {		
				doi = "N/A";
				};

				email = data[i].email_address;
				if (email == "not available") {		
				email = "N/A";
				};

				title1 = data[i].article_title;
				if (title1 == "not available") {		
				title1 = "<Please click for title>";
				};

			var textnode = document.createTextNode("The term " + "\"" + input + "\"" + " is linked to an article entitled " + 
				"\"" + title1 + "\"" + " that also contains the SNP identifier " + rsID + ". This SNP identifier was mentioned at least " + cited + 
				" time(s) in the article published in " + year + ". The article has also been tagged with Digital Object Identifier (DOI) " +  doi  + 
				" and with Pubmed Central ID " +  pub + 
				". For more information contact the correspondence author at " + email +
				" or click here to view the full paper");
			
			elem.appendChild(textnode);
			elem.title = "Read the full paper";
			elem.href = "http://www.ncbi.nlm.nih.gov/pmc/?term=" + pub;
			elem.target = "_blank"
			elem.id = "updated";

			listz.push([cited, elem]);
			listz.sort(function(a, b){return a[0]-b[0]});		

			};
			
			while (listz.length != 0) {
			readyToGo = listz.pop();
			readyToGo = readyToGo.concat();
			$("rs").append(readyToGo[1]);
			$("[id=updated]").addClass("btn btn-default updateSpecs");
			};

			});
			
			

	});
			$("#clearButton").on("click", function() {
				$("#divz").children().empty();
				$("#box").show();
			});
});	


